...
class BNG_Custom_Payment_Gateway extends WC_Payment_Gateway {
    public function __construct() {
        ...
        if ( (ngfw_fs()->is_not_paying() || ngfw_fs()->is_free_plan()) && Config::$projectType == 'nmi') {
            $this->supports = array(
                'refunds',
                'tokenization'
            );
    ...
    function admin_options() {
        ?>
            <h3><?php _e('NMI Gateway For WooCommerce', Config::$pluginId); ?></h3>
    ...
    function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled'           => array(
                ...
                'type'          => 'checkbox',
                'label'         => __( 'Enable NMI Gateway For WooCommerce', Config::$pluginId ),
                'default'       => 'no',
            ),
            'title'             => array(
                ...
                'default'       => __( 'Credit Card / ECheck (NMI)', Config::$pluginId ),
                'desc_tip'      => true,
            ),
            'description'       => array(
    ...
    function get_saved_payment_method_list($savedMethods) {
        if (empty($savedMethods) || ngfw_fs()->is_plan('Premium') || Config::$projectType === 'bng') {
    ...
    function receipt_page( $order_id ) {
        ...
        if ($this->apikey != '') {
            ...
            if (ngfw_fs()->is_plan('Premium') || Config::$projectType == 'bng') {
    ...
    function add_admin_change_payment_method_form($payment_meta, $subscription) {
        if ( class_exists('WC_Subscriptions') && (ngfw_fs()->is_plan('Premium') || Config::$projectType === 'bng') ) {
    ...
    function successful_request($tokenid, $orderid) {
        ...
        if ( class_exists('WC_Subscriptions') && (Config::$projectType == 'bng' || ngfw_fs()->is_plan('Premium')) ) {
...
function bng701_complete_order($order, $status, $result, $trans_type, $bngGatewayObj, $isAcctScreen = false) {
    ...
    if ($status == 'success') {
        if ($isAcctScreen) {
            ...
            if ( function_exists('wcs_is_subscription') && !wcs_is_subscription($order) || !function_exists('wcs_is_subscription')) {
                ...
                if ($trans_type == 'auth') {
                    ...
                    $order->add_order_note( __( 'NMI Gateway payment authorization completed.', Config::$pluginId ) );
                    $order->set_transaction_id($result['transaction-id']);
                    $order->save();
                }
                else {
                    $order->add_order_note( __( 'NMI Gateway payment completed.', Config::$pluginId ) );
                ...
                if (function_exists('wc_add_notice')) wc_add_notice(__('Your order is complete! Thank you!', Config::$pluginId), $status);
            }
        }
    }
    else {
        ...
        if ($isAcctScreen) {
            ...
            if (function_exists('wc_add_notice')) wc_add_notice( __( "Failure: Payment method could not be changed on subscription {$order->get_id()}.", Config::$pluginId ), $status );
        }
        else {
            // Add helpful notes
            ...
            if (function_exists('wc_add_notice')) wc_add_notice(__('An error occurred completing order: ' . $dsp_error, Config::$pluginId), $status);
...
function bng701_html($savepaymentmethodtoggle, $tokenizationkey='', $data='', $order_id='', $paymentMethods='') {
    ...
    if (Config::$projectType == 'bng' || ngfw_fs()->is_plan('Premium') ) {
        ...
        if ( function_exists('wcs_is_subscription') && wcs_is_subscription($order_id) ) $pageMessage = "Select Desired Payment Method via the NMI Payment Gateway";
...
function bng701_payment_method_change_notification($token, $order) {
    ...
    if (function_exists('wc_add_notice')) wc_add_notice( __( "Payment method successfully changed on subscription {$order->get_id()}.", Config::$pluginId ) );
}
...
function bng701_remove_payment_meta_from_suscription($subscription) {
    ...
    if (function_exists('wc_add_notice')) wc_add_notice(__("Payment method has been deleted from subscription - {$id}", Config::$pluginId));
}
